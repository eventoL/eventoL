...
class Sede(models.Model):
    ...
    def __unicode__(self):
        return "%s / %s / %s - %s" % (
        self.country, self.state, self.city, self.name)
...
class Attendant(models.Model):
    ...
    is_going_to_install = models.BooleanField(
        _('Is going to install?'), default=False,
        help_text=_('Are you going to bring a PC for installing it?')
    )
    additional_info = models.TextField(
        _('Additional Info'), blank=True, null=True,
...
class Organizer(models.Model):
    ...
    is_coordinator = models.BooleanField(
        _('Is Coordinator'), default=False,
    ...
    assignation = models.CharField(
        _('Assignation'), max_length=200, blank=True, null=True,
        help_text=_('Assignations given to the user (i.e. Talks, Coffee...)')
    )
    additional_info = models.CharField(
        _('Additional Info'), max_length=200, blank=True, null=True,
        help_text=_('Any additional info you consider relevant')
    )
    assisted = models.BooleanField(_('Assisted'), default=False)
    time_availability = models.CharField(
        _('Time Availability'), max_length=200, blank=True, null=True,
        help_text=_(
...
class Hardware(models.Model):
    ...
    manufacturer = models.ForeignKey(
        HardwareManufacturer, verbose_name=_('Manufacturer'),
        blank=True, null=True
    )
...
class Installer(Organizer):
    level = models.CharField(_('Level'), choices=(('1', _('Beginner')),
                                                  ...
                                                  ('3', _('Advanced')),
                                                  ('4', _('Super Hacker')),),
                             max_length=200, help_text=_(
            'Linux Knowledge level for an installation'))
    software = models.ManyToManyField(
        Software, verbose_name=_('Software'), blank=True, null=True,
        help_text=_(
...
class Installation(models.Model):
    attendant = models.ForeignKey(
        Attendant, verbose_name=_('Attendant'),
    ...
    installer = models.ForeignKey(
        Installer, verbose_name=_('Installer'),
        related_name='installed_by', blank=True, null=True
    )
...
class TalkProposal(models.Model):
    ...
    sede = models.ForeignKey(
        Sede, verbose_name=_noop('Sede'),
        help_text=_('Sede you are proposing the talk to')
    )
    presentation = models.FileField(
        _('Presentation'), upload_to='talks', blank=True, null=True,
        help_text=_('Any material you are going to use for the talk (optional, but recommended)')
    )
    type = models.ForeignKey(TalkType, verbose_name=_('Type'))
    home_image = ImageCropField(
        upload_to='talks_thumbnails', verbose_name=_('Home Page Image'),
        blank=True, null=True,
        help_text=_('Image that is going to appear in the home page of this web for promoting the talk (optional)')
    )
    cropping = ImageRatioField(
        'home_image', '700x450', size_warning=True, verbose_name=_('Cropping'),
...
class Room(models.Model):
    ...
    for_type = models.ForeignKey(
        TalkType, verbose_name=_('For talk type'),
...
class TalkTime(models.Model):
    ...
    talk_type = models.ForeignKey(
        TalkType, verbose_name=_('Talk Type'),
        help_text=_( 'The type of talk the room is going to be used for.')
    )
