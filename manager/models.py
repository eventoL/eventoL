...
class Activity(models.Model):
    ...
    def room_available(cls, request=None, instance=None, event_slug=None, error=False):
        ...
        if activities_room.filter(
                end_date__range=(instance.start_date + one_second, instance.end_date - one_second)).exists() \
                or activities_room.filter(end_date__gt=instance.end_date, start_date__lt=instance.start_date).exists() \
                or activities_room.filter(
                    start_date__range=(instance.start_date + one_second, instance.end_date - one_second)).exists() \
                or activities_room.filter(end_date=instance.end_date, start_date=instance.start_date).exists():
