...
class EventManager(models.Manager):
    def get_queryset(self):
        ...
        return (
            ...
            .annotate(
                activity_proposal_is_open=models.Case(
            ...
            .annotate(
                registration_is_open=models.Case(
...
class EventTag(models.Model):
    ...
    background = models.ImageField(null=True, blank=True, help_text=_("A image to show in the background of"))
    logo_header = models.ImageField(
        null=True,
        blank=True,
        help_text=_("This logo will be shown in the right corner of the page"),
    )
    logo_landing = models.ImageField(null=True, blank=True, help_text=_("Logo to show in the center of the page"))
    ...
    slug = models.SlugField(_("URL"), max_length=100, help_text=_("For example: flisol-caba"), unique=True)
...
class Event(models.Model):
    ...
    registration_closed = models.BooleanField(default=False, help_text=_("set it to True to force the registration to be closed"))

    tags = models.ManyToManyField(EventTag, blank=True, help_text=_("Select tags to show this event in the EventTag landing"))
    event_slug = models.SlugField(_("URL"), max_length=100, help_text=_("For example: flisol-caba"), unique=True)
    ...
    event_information = RichTextField(verbose_name=_("Event Info"), help_text=_("Event Info HTML"), blank=True, null=True)
...
class EventDate(models.Model):
    event = models.ForeignKey(Event, verbose_name=_noop("Event"), blank=True, null=True, on_delete=models.CASCADE)
...
class Contact(models.Model):
    ...
    def __str__(self):
        return f"{self.event} - {self.type} - {self.text}"
...
class EventUserManager(models.Manager):
    ...
    def get_counts(event_users):
        confirmed = EventUserAttendanceDate.objects.filter(event_user__in=event_users).order_by("event_user").distinct().count()
        total = len(event_users)
        return {"total": total, "confirmed": confirmed, "not_confirmed": total - confirmed}

    def get_counts_by_event(self, event):
        model = self.model
        queryset = model.objects.filter(event_user__event=event) if hasattr(model, "event_user") else model.objects.filter(event=event)
...
class EventUser(models.Model):
    ...
    def __str__(self):
        if self.user:
            return f"{self.user.username} at event:{self.event}"
        return f"{self.event}"
    ...
    def attended_today(self):
        return EventUserAttendanceDate.objects.filter(event_user=self, date__date=timezone.localdate()).exists()
...
class EventUserAttendanceDate(models.Model):
    ...
    def __str__(self):
        return f"{self.event_user} - {self.date}"
...
class Collaborator(models.Model):
    ...
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
    updated_at = models.DateTimeField(_("Updated At"), auto_now=True)
    event_user = models.ForeignKey(EventUser, verbose_name=_("Event User"), blank=True, null=True, on_delete=models.CASCADE)
...
class AttendeeManager(EventUserManager):
    ...
    def get_counts(self, queryset):
        event_users = self.get_event_users(queryset.filter(event_user__isnull=False))
        confirmed_with_event_user = (
            AttendeeAttendanceDate.objects.filter(attendee__event_user__in=event_users).order_by("attendee__event_user").distinct().count()
        )
        total_with_event_user = len(event_users)
        attendees = self.get_attendee(queryset)
        confirmed = (
            AttendeeAttendanceDate.objects.filter(attendee__in=attendees, attendee__event_user__isnull=True).order_by("attendees").distinct().count()
        )
        total = len(attendees)
        return {
            "with_event_user": {
                ...
                "not_confirmed": total_with_event_user - confirmed_with_event_user,
            },
            "without_event_user": {
                "total": total,
                "confirmed": confirmed,
...
class Attendee(models.Model):
    ...
    event_user = models.ForeignKey(EventUser, verbose_name=_noop("Event User"), blank=True, null=True, on_delete=models.CASCADE)
    ...
    def __str__(self):
        if self.event_user:
            return str(self.event_user)
        return f"{self.event} - {self.first_name} {self.last_name} - {self.nickname} - {self.email}"
    ...
    @property
    def attended_today(self):
        return AttendeeAttendanceDate.objects.filter(attendee=self, date__date=timezone.localdate()).exists()


class AttendeeAttendanceDate(models.Model):
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
    updated_at = models.DateTimeField(_("Updated At"), auto_now=True)
    attendee = models.ForeignKey(Attendee, verbose_name=_noop("Attendee"), blank=False, null=False, on_delete=models.CASCADE)
    date = models.DateTimeField(_("Date"), help_text=_("The date of the attendance"), auto_now_add=True)
    attendance_mode_choices = (
        ("1", _("Qr autoregistration")),
        ("2", _("Qr ticket")),
    ...
    def __str__(self):
        return f"{self.attendee} - {self.date}"
...
class InstallationMessage(models.Model):
    ...
    message = RichTextField(
        ...
        help_text=_("Email message HTML Body"),
        blank=True,
        null=True,
    )
...
class Activity(VoteModel, models.Model):
    ...
    justification = models.TextField(_("Justification"), blank=True, null=True, help_text=_("Why do you reject this proposal?"))
    room = models.ForeignKey(Room, verbose_name=_("Room"), blank=True, null=True, on_delete=models.CASCADE)
    ...
    speakers_names = models.CharField(_("Speakers Names"), max_length=600, help_text=_("Comma separated speaker names"))
    ...
    level = models.CharField(_("Level"), choices=level_choices, max_length=100, help_text=_("Talk's Technical level"))
    ...
    status = models.CharField(_("Status"), choices=status_choices, max_length=20, help_text=_("Activity proposal status"))
    ...
    @classmethod
    def room_available(cls, request, proposal, event_slug, event_date, error=False):
        activities_room = Activity.objects.filter(room=proposal.room, event__event_slug=event_slug, start_date__date=event_date)
        if proposal.start_date == proposal.end_date:
            message = _(
                "The talk couldn't be registered because the schedule is not \
        ...
        if (
            activities_room.filter(end_date__range=(proposal.start_date + one_second, proposal.end_date - one_second))
            ...
            or activities_room.filter(start_date__range=(proposal.start_date + one_second, proposal.end_date - one_second))
            .exclude(pk=proposal.pk)
            .exists()
            or activities_room.filter(end_date=proposal.end_date, start_date=proposal.start_date).exclude(pk=proposal.pk).exists()
        ):
            message = _(
                "The talk couldn't be registered because the \
...
class InstallationManager(models.Manager):
    @staticmethod
    def get_counts(queryset):
        hardware_count = count_by(queryset, lambda activity: activity.hardware.model)
        software_count = count_by(queryset, lambda activity: activity.software.name)
        installer_count = count_by(queryset, lambda activity: activity.installer.id)
        total = queryset.count()
        return {
...
class Installation(models.Model):
    ...
    created_at = models.DateTimeField(_("Created At"), auto_now_add=True)
    updated_at = models.DateTimeField(_("Updated At"), auto_now=True)
    hardware = models.ForeignKey(Hardware, verbose_name=_("Hardware"), blank=True, null=True, on_delete=models.CASCADE)
    software = models.ForeignKey(Software, verbose_name=_("Software"), blank=True, null=True, on_delete=models.CASCADE)
    ...
    def __str__(self):
        return f"{self.attendee}, {self.hardware}, {self.software}"
