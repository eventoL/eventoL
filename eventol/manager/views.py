...
from manager.forms import ActivityDummyForm
from manager.forms import ActivityForm
from manager.forms import ActivityProposalForm
from manager.forms import AttendeeRegistrationByCollaboratorForm
from manager.forms import AttendeeRegistrationForm
from manager.forms import AttendeeRegistrationFromUserForm
from manager.forms import AttendeeSearchForm
from manager.forms import CollaboratorRegistrationForm
from manager.forms import ContactForm
from manager.forms import ContactMessageForm
from manager.forms import EventDateForm
from manager.forms import EventDateModelFormset
from manager.forms import EventForm
from manager.forms import EventImageCroppingForm
from manager.forms import EventUserRegistrationForm
from manager.forms import EventUserSearchForm
from manager.forms import HardwareForm
from manager.forms import ImageCroppingForm
from manager.forms import InstallationForm
from manager.forms import InstallerRegistrationForm
from manager.forms import RejectForm
from manager.forms import RoomForm
...
from manager.models import InstallationMessage
...
from manager.security import add_organizer_permissions
from manager.security import are_activities_public
from manager.security import is_activity_public
from manager.security import is_collaborator
from manager.security import is_collaborator_or_installer
from manager.security import is_installer
from manager.security import is_organizer
from manager.security import is_reviewer
from manager.security import is_speaker
from manager.security import user_passes_test
...
def generate_ticket(user):
    ...
    ticket_template.set_text("eventUser_PK", str(ticket_data["ticket"]))
    ...
    if exists_first_name or exists_last_name:
        user_name_l1 = "{} {}".format(ticket_data["first_name"], ticket_data["last_name"])
        user_name_l2 = ""
...
def index(request, event_slug):
    ...
    activities_list = Activity.objects.filter(event=event, status=2).exclude(image__isnull=True).exclude(is_dummy=True).distinct()
...
def home(request):
    if "registration_event_slug" in request.session and request.user.is_authenticated:
        ...
        event_index = reverse(f"{role}_registration", args=[event_slug])
                ...
                if postinstall_email:
                    try:
                        ...
                        messages.error(
                            request,
                            _(
                                "The email couldn't sent successfully, \
                                                    please retry later or contact a organizer"
                            ),
                        )
                messages.success(
                    request,
                    _("The installation has been registered successfully. " "Happy Hacking!"),
                )
        ...
        if attendee_form.is_valid():
            ...
            if attendee:
                if attendee.attended_today:
                    ...
                    messages.success(
                        request,
                        _("The attendee has been successfully registered. " "Happy Hacking!"),
                    )
        ...
        if collaborator_form.is_valid():
            ...
            if event_user:
                if event_user.attended_today:
                    ...
                    messages.success(
                        request,
                        _("The collaborator has been successfully " "registered. Happy Hacking!"),
                    )
                return redirect(reverse("manage_attendance", args=[event_slug]))
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request,
                    (f"{organizer.event_user.user.username} has been successfully added as an Organizer."),
                )
            return redirect(reverse("add_organizer", args=[event_slug]))

        messages.error(request, _("Something went wrong (please check form errors)"))

    organizers = Organizer.objects.filter(event_user__event__event_slug=event_slug)
    return render(
        request,
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request,
                    _("{} has been successfully added to manage attendance.").format(event_user.user.username),
                )
            return redirect(reverse("add_registration_people", args=[event_slug]))

        messages.error(request, _("Something went wrong (please check form errors)"))

    content_type = ContentType.objects.get_for_model(Attendee)
    permission = Permission.objects.filter(codename=CAN_TAKE_ATTENDANCE_PERMISSION_CODE_NAME, content_type=content_type).first()
    registration_people = []

    if permission is not None:
        registration_people = Collaborator.objects.filter(event_user__user__user_permissions=permission, event_user__event__event_slug=event_slug)
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request,
                    _("{} has been successfully added as reviewer.".format(event_user.user.username)),
                )
...
def registration_from_installation(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    installation_url = reverse("installation", args=[event_slug])
    render_template = "registration/attendee/from-installation.html"
    return process_attendee_registration(request, event=event, return_url=installation_url, render_template=render_template)
...
def registration_by_collaborator(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    manage_attendance_url = reverse("manage_attendance", args=[event_slug])
    render_template = "registration/attendee/by-collaborator.html"
    return process_attendee_registration(request, event=event, return_url=manage_attendance_url, render_template=render_template)


def process_attendee_registration(request, event, return_url, render_template):
    # Verify date, allow only on event day or after
    eventdate = EventDate.objects.filter(event=event, date__lte=timezone.localdate()).order_by("-date").first()
    ...
    if eventdate:
        if request.POST:
            if form.is_valid():
                ...
                if Attendee.objects.filter(event=event, email__iexact=email).exists():
                    messages.error(
                        request,
                        _("The attendee is already registered for this event, " "use correct form"),
                    )
                    return redirect(return_url)
                with transaction.atomic():
                    attendee = form.save()
                    AttendeeAttendanceDate.objects.create(attendee=attendee, mode="4")
        ...
        messages.error(
            request,
            _(
                "You can only register an attendance \
                                  at the day of the event or after"
            ),
        )
    return render(request, render_template, update_event_info(event.event_slug, {"form": form}))
...
def attendee_registration_print_code(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    self_registration_url = request.build_absolute_uri(reverse("attendee_registration_by_self", args=[event_slug, event.registration_code]))
    ...
    registration_code_template = svglue.load(file=os.path.join(settings.STATIC_ROOT, "manager/img/registration_code_template_p.svg"))
    ...
    response = HttpResponse(svg2pdf(bytestring=registration_code_svg), content_type="application/pdf")
    response["Content-Disposition"] = f"filename=Registration-code-{event.event_slug}.pdf"
    return response
    ...
    if request.POST:
        ...
        if attendee:
            ...
            with transaction.atomic():
                ...
                return redirect(event_index_url)
        messages.error(request, _("You couldn't be registered (check form errors)"))
    return render(
        request,
...
def attendance_by_autoreadqr(request, event_slug):
    ...
    if (
        not event_registration_code
        and user.is_authenticated
        and (is_collaborator(user, event_slug=event_slug) or is_organizer(user, event_slug=event_slug))
    ):
        return redirect(
    ...
    if not event_registration_code or not Event.objects.filter(event_slug=event_slug, registration_code=event_registration_code).exists():
        messages.error(
            request,
            _(
                "The registration code does \
    ...
    if ticket_code:
        ...
        if event_user.exists():
            ...
            attendance = EventUserAttendanceDate.objects.filter(event_user=event_user, date__date=timezone.localdate())
            ...
            attendance = AttendeeAttendanceDate.objects.filter(attendee=attendee, date__date=timezone.localdate())
            if attendance.exists():
                messages.info(request, _("You are already registered and present! Go have fun"))
            else:
...
def contact(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            email.subject = _noop("[eventoL] Contact message from {name}").format(name=contact_message.name)
        ...
        messages.error(
            request,
            _("There was a problem sending your message. " "Please try again in a few minutes."),
        )
...
def reports(request, event_slug):
    ...
    confirmed_collaborators_count = (
        EventUserAttendanceDate.objects.filter(event_user__event=event, event_user__in=collaborators_event_users)
        .order_by("event_user")
        .distinct()
        .count()
    )
    not_confirmed_collaborators = collaborators.count() - confirmed_collaborators_count
    confirmed_installers_count = (
        EventUserAttendanceDate.objects.filter(event_user__event=event, event_user__in=installers_event_users)
        .order_by("event_user")
        .distinct()
        .count()
    )
...
def get_email_confirmation_url(request, event_slug, attendee_id, token):
    url = reverse("attendee_confirm_email", args=[event_slug, attendee_id, token])
...
def attendee_registration(request, event_slug):
    ...
    if not event.registration_is_open or event.registration_closed:
        return render(
            ...
            update_event_info(event_slug, event=event),
        )
    ...
    if request.user.is_authenticated:
        ...
        attendee = Attendee(
            ...
            email=event_user.user.email,
            event=event_user.event,
            nickname=event_user.user.username,
        )
        attendee_form = AttendeeRegistrationFromUserForm(request.POST or None, instance=attendee)
    else:
        attendee_form = AttendeeRegistrationForm(request.POST or None, initial={"event": event})
    ...
    if request.POST:
        if attendee_form.is_valid():
            with transaction.atomic():
                ...
                confirm_url = get_email_confirmation_url(request, event_slug, attendee.id, attendee.email_token)
                ...
                email.send(fail_silently=False)
                return redirect(reverse("attendee_email_sent", args=[event_slug]))
...
def installer_registration(request, event_slug):
    ...
    return generic_registration(request, event_slug, Installer, InstallerRegistrationForm, msg_success, msg_error, template)
...
def create_event(request):
    ...
    contacts_formset = contacts_formset(request.POST or None, prefix="contacts-form", queryset=Contact.objects.none())
    # pylint: enable=unexpected-keyword-arg

    event_date_formset = modelformset_factory(EventDate, form=EventDateForm, formset=EventDateModelFormset, can_delete=True)

    # pylint: disable=unexpected-keyword-arg
    event_date_formset = event_date_formset(request.POST or None, prefix="event-date-form", queryset=EventDate.objects.none())
    # pylint: enable=unexpected-keyword-arg
...
def edit_event(request, event_slug):
    ...
    contacts_formset = modelformset_factory(Contact, form=ContactForm, can_delete=True)
    # pylint: disable=unexpected-keyword-arg
    contacts_formset = contacts_formset(request.POST or None, prefix="contacts-form", queryset=event.contacts.all())
    # pylint: enable=unexpected-keyword-arg
...
def view_ticket(request, event_slug):
    event_user = EventUser.objects.filter(event__event_slug=event_slug).filter(user=request.user).first()
    if event_user:
...
def draw(request, event_slug):
    users = [
        str(attendance_date.attendee)
        for attendance_date in AttendeeAttendanceDate.objects.filter(attendee__event__event_slug=event_slug, date__date=timezone.localdate())
    ]
    return render(
        request,
        "event/draw.html",
        update_event_info(event_slug, {"eventusers": users, "eventusersjson": json.dumps(users)}),
    )
...
def activity_dummy(request, event_slug):
    ...
    activity = Activity(event=event, status="2", owner=event_user, is_dummy=True, activity_type=activity_type)
    activity_form = ActivityDummyForm(request.POST or None, instance=activity)
    if request.POST:
        if activity_form.is_valid():
            try:
                activity = activity_form.save()
                return redirect(reverse("activities", args=[event_slug]))
            except Exception as error_message:
                logger.error(error_message)
        ...
        messages.error(
            request,
            _(
                "There was a problem submitting the proposal. \
    ...
    return render(
        ...
        update_event_info(event_slug, {"form": activity_form, "errors": [], "multipart": True}, event=event),
    )
...
def image_cropping(request, event_slug, activity_id):
    ...
    if request.POST:
        if form.is_valid():
            ...
            return redirect(reverse("activity_detail", args=[event_slug, activity.pk]))
        messages.error(
            request,
            _(
                "The proposal couldn't be registered. Please \
        check the form for errors"
            ),
        )
    return render(request, "activities/image-cropping.html", update_event_info(event_slug, {"form": form}))
...
def activities(request, event_slug):
    ...
    for activity in list(dummy_activities) + list(activities_instances):
        activity.form = ActivityForm(event_slug, instance=activity)
        activity.reject_form = RejectForm()
        activity.errors = []
    return render(
        request,
        "activities/activities_home.html",
        update_event_info(
            event_slug,
            {
                "emails": emails,
...
def talk_registration(request, event_slug, proposal_id):
            ...
            if talk_form.is_valid():
                ...
                if room_available:
                    try:
                        ...
                        safe_continue = reverse("activity_detail", args=[event_slug, proposal.pk])
                        return goto_next_or_continue(request.GET.get("next"), safe_continue)
                    except SMTPException as error:
                        logger.error(error)
                        messages.error(
                        ...
                        messages.error(
                            request,
                            _(
                                "The talk couldn't be registered, \
    ...
    return render(request, "activities/detail.html", update_event_info(event_slug, render_dict))
...
def event_add_image(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            if request.FILES:
                ...
                event.save()
                messages.success(request, _("Please crop or upload a new image."))
                return redirect(reverse("event_add_image", args=[event_slug]))
            form.save()
            messages.success(
                request,
                _("The event has been updated successfully! We'll contact you at the provided email"),
            )
            return redirect(reverse("index", args=[event_slug]))
        messages.error(
            request,
    ...
    return render(request, "event/image-cropping.html", update_event_info(event_slug, {"form": form}))
...
def activity_detail(request, event_slug, activity_id):
    ...
    return render(request, "activities/detail.html", update_event_info(event_slug, params))


def schedule(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    event_dates = event.eventdate_set.order_by("date")
    activities_count = Activity.objects.filter(event=event).filter(room__isnull=False).filter(status="2").order_by("start_date").count()
    ...
    for event_date in event_dates:
        activities_dict[event_date.date.strftime("%Y%m%d")] = (
            Activity.objects.filter(event=event, start_date__date=event_date.date)
            ...
            schedule_activities[date] = json.dumps(
                {
                    ...
                    "max_time": sorted(activities_for_date, key=lambda o: o.end_date.time())[-1].end_date.time().strftime("%H:%M"),
...
def rooms_list(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    rooms = Room.objects.filter(event=event)
    return render(request, "rooms/list.html", update_event_info(event_slug, {"rooms": rooms}))
    ...
    if request.POST:
        if room_form.is_valid():
            with transaction.atomic():
                ...
                room.save()
                if is_edit:
                    messages.success(request, _("The Room has been edited successfully."))
                else:
                    messages.success(request, _("The Room has been added successfully."))
                return redirect(reverse("rooms_list", args=[event_slug]))
        if is_edit:
            messages.error(request, _("The room couldn't be edited (check form errors)"))
        else:
            messages.error(request, _("The room couldn't be added (check form errors)"))
        errors = get_forms_errors(forms)
    return render(
        request,
...
def delete_room(request, event_slug, room_id):
    ...
    if activities_instances.exists():
        messages.error(request, _("The room has assigned activities."))
    else:
        room.delete()
...
def generic_report(request):
    ...
    return render(
        request,
        "generic-report.html",
        {"events_private_data": events_private_data, "ws_propocol": settings.WS_PROTOCOL},
    )
