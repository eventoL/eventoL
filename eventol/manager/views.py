...
def index(request, event_slug):
    ...
    activities_list = (
        Activity.objects.filter(event=event, status=2).exclude(image__isnull=True).exclude(is_dummy=True).distinct()
    )
...
def add_organizer(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request, _('%s has been successfully added as an Organizer.' % organizer.event_user.user.username)
                )
    ...
    return render(
        request, 'event/organizers.html', update_event_info(event_slug, {'form': form, 'organizers': organizers})
    )
...
def add_registration_people(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request, _('%s has been successfully added to manage attendance.' % event_user.user.username)
                )
    ...
    if permission is not None:
        registration_people = Collaborator.objects.filter(
            event_user__user__user_permissions=permission, event_user__event__event_slug=event_slug
        )
...
def add_reviewer(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            if event_user:
                ...
                messages.success(
                    request, _('{} has been successfully added as reviewer.'.format(event_user.user.username))
                )
    ...
    return render(
        request, 'event/review_people.html', update_event_info(event_slug, {'form': form, 'review_people': reviewers})
    )
...
def registration_from_installation(request, event_slug):
    ...
    return process_attendee_registration(
        request, event=event, return_url=installation_url, render_template=render_template
    )
...
def registration_by_collaborator(request, event_slug):
    ...
    return process_attendee_registration(
        request, event=event, return_url=manage_attendance_url, render_template=render_template
    )
...
def attendee_registration_print_code(request, event_slug):
    event = get_object_or_404(Event, event_slug=event_slug)
    self_registration_url = request.build_absolute_uri(
        reverse('attendee_registration_by_self', args=[event_slug, event.registration_code])
    )
    ...
    response['Content-Disposition'] = 'filename=Registration-code-{event}.pdf'.format(event=event.event_slug)
    return response
...
def attendance_by_autoreadqr(request, event_slug):
    ...
    if (
        not event_registration_code
        or not Event.objects.filter(event_slug=event_slug, registration_code=event_registration_code).exists()
    ):
        messages.error(
            request,
            _(
...
def reports(request, event_slug):
    ...
    for event_date in event_dates:
        attendance_for_date = (
            AttendeeAttendanceDate.objects.filter(attendee__event=event, date__date=event_date.date)
            .order_by('attendee')
            .distinct()
        )
...
def generic_registration(request, event_slug, registration_model, new_role_form, msg_success, msg_error, template):
    ...
    return render(
        request, template, update_event_info(event_slug, {'forms': forms, 'errors': errors, 'multipart': False})
    )
...
def attendee_registration(request, event_slug):
    ...
    if not event.registration_is_open or event.registration_closed:
        return render(request, 'registration/closed-registration.html', update_event_info(event_slug, event=event))
    ...
    return render(
        ...
        update_event_info(
            event_slug,
            {'form': attendee_form, 'errors': get_forms_errors([attendee_form]), 'multipart': False},
            event=event,
        ),
    )
...
def installer_registration(request, event_slug):
    ...
    return generic_registration(
        request, event_slug, Installer, InstallerRegistrationForm, msg_success, msg_error, template
    )
...
def collaborator_registration(request, event_slug):
    ...
    return generic_registration(
        request, event_slug, Collaborator, CollaboratorRegistrationForm, msg_success, msg_error, template
    )
...
def create_event(request):
    ...
    event_date_formset = event_date_formset(
        request.POST or None, prefix='event-date-form', queryset=EventDate.objects.none()
    )
    # pylint: enable=unexpected-keyword-arg
...
def edit_event(request, event_slug):
    ...
    # pylint: disable=unexpected-keyword-arg
    event_date_formset = event_date_formset(
        request.POST or None, prefix='event-date-form', queryset=EventDate.objects.filter(event=event)
    )
    # pylint: enable=unexpected-keyword-arg
...
def draw(request, event_slug):
    users = [
        ...
        for attendance_date in AttendeeAttendanceDate.objects.filter(
            attendee__event__event_slug=event_slug, date__date=timezone.localdate()
        )
    ]
    return render(
        request,
        ...
        if isinstance(start_time, datetime.time) or isinstance(end_time, datetime.time):
            ...
            if talk_form.is_valid():
                room_available = Activity.room_available(
                    request=request, proposal=talk_form.instance, event_slug=event_slug, event_date=event_date.date
                )
                if room_available:
                    try:
                        proposal.status = 2
...
def event_add_image(request, event_slug):
    ...
    if request.POST:
        if form.is_valid():
            ...
            form.save()
            messages.success(
                request, _("The event has been updated successfully! We'll contact you at the provided email")
            )
            return redirect(reverse('index', args=[event_slug]))
        messages.error(
            request,
...
def schedule(request, event_slug):
    ...
    if not event.schedule_confirmed or activities_count <= 0:
        return render(request, 'activities/schedule_not_confirmed.html', update_event_info(event_slug, {}, event=event))
            ...
            schedule_activities[date] = json.dumps(
                {
                    ...
                    'max_time': sorted(activities_for_date, key=lambda o: o.end_date.time())[-1]
                    .end_date.time()
                    .strftime('%H:%M'),
                    'date': activities_for_date.first().start_date.date().isoformat(),
                    'datestring': date_format(
                        activities_for_date.first().start_date, format='SHORT_DATE_FORMAT', use_l10n=True
                    ),
                }
            )
...
def add_or_edit_room(request, event_slug, room_id=None):
    ...
    return render(
        ...
        update_event_info(
            event_slug,
            {'is_edit': is_edit, 'room': room if room else None, 'forms': forms, 'errors': errors, 'multipart': False},
        ),
    )
