...
def get_activity_body(event_name, activity_title, activity_status, justification=None):
    justification_txt = _("Justification: {}\n").format(justification) if justification is not None else ""
    body_txt = _(
        "Hello,\n"
...
def get_ticket_body(first_name, last_name, event_name):
    body_txt = _(
        ...
        "Regards,\n"
        "{event_name} and eventoL team"
    ).format(first_name=first_name, last_name=last_name, event_name=event_name)
    body_html = _(
...
def get_installation_subject(first_name, last_name, event_name):
    return _("{first_name} {last_name}, thank you for participating {event_name}").format(
...
def send_ticket_email(ticket_data, ticket_svg):
    ...
    email.attach_alternative(body_html, "text/html")
    email.to = [email_to]
    email.attach(f"Ticket-{ticket_code}.pdf", cairosvg.svg2pdf(bytestring=ticket_svg), "application/pdf")
    email.send(fail_silently=False)
