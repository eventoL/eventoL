...
export default class ReportTable extends React.PureComponent {
      ...
      columns: [
        {
          ...
          total: totals[total_category].confirmed,
          Footer: (
            <span>
              <strong> {gettext('Total')}: </strong>
              {totals[total_category].confirmed}
            </span>
          ),
        },
        {
          ...
          total: totals[total_category].not_confirmed,
          Footer: (
            <span>
              <strong> {gettext('Total')}: </strong>
              {totals[total_category].not_confirmed}
            </span>
          ),
        },
        {
          ...
          total: totals[total_category].total,
          Footer: (
            <span>
              <strong> {gettext('Total')}: </strong>
              {totals[total_category].total}
            </span>
          ),
        },
      ],
  ...
  getAssistancesFullColumns() {
    ...
    if (!totals.hasOwnProperty('attendees')) return {};
    return {
      attendees: this.getConfirmationColumnsBy(
        'attendees',
        'attendees',
        'assistanceDetail',
        'Attendees'
      ),
      collaborators: this.getConfirmationColumnsBy(
        'collaborator',
        'collaborators',
        'report',
        'Collaborators'
      ),
      installers: this.getConfirmationColumnsBy(
        'installer',
        'installers',
        'report',
        'Installers'
      ),
      organizers: this.getConfirmationColumnsBy(
        'organizer',
        'organizers',
        'report',
        'Organizers'
  ...
  getActivitiesFieldsColumns(category, plural_category, names) {
    const {totals} = this.props;
    if (!totals.hasOwnProperty('activities')) return [];
    return Object.keys(totals.activities.details[plural_category]).map(
      element => {
        const Header =
        ...
        return {
          ...
          total: totals.activities.details[plural_category][element] || 0,
          Footer: (
            <span>
              <strong> {gettext('Total')}: </strong>
              {totals.activities.details[plural_category][element] || 0}
            </span>
          ),
        };
      }
  ...
  getActivitiesFullColumns() {
    const confirmationColumn = this.getConfirmationColumnsBy(
      'activity',
      'activities',
      'report',
      'Attendees'
    ...
    const statusColumns = this.getActivitiesFieldsColumns('status', 'status', [
      gettext('Proposal'),
      gettext('Accepted'),
      gettext('Rejected'),
    ]);
    const typeColumns = this.getActivitiesFieldsColumns('type', 'types', [
      gettext('Talk'),
      gettext('Workshop'),
      gettext('Lightning talk'),
      gettext('Other'),
    ]);
    const levelColumns = this.getActivitiesFieldsColumns('level', 'levels', [
      gettext('Beginner'),
      gettext('Medium'),
      gettext('Advanced'),
    ]);
